...
def calculate_loss_given_default(loan, client_securities):
    ...
    if not loan or not hasattr(loan, 'outstanding_loan_balance') or not loan.outstanding_loan_balance:
        return default_lgd
...
def calculate_marginal_ecl(loan, pd, lgd, eir, reporting_date):
    ...
    if not loan or not hasattr(loan, 'outstanding_loan_balance') or loan.outstanding_loan_balance is None:
        return Decimal('0.0')
    ...
    if eir_decimal > Decimal('0'):
        pv_factor = Decimal('1.0') / ((Decimal('1.0') + eir_decimal) ** Decimal(str(years_to_maturity)))
    else:
