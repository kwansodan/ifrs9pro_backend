...
def find_missing_repayment_data(db: Session, portfolio_id: int) -> List[Dict]:
    ...
    for loan in loans:
        ...
        if (
            loan.loan_issue_date
            and loan.maturity_period
            and (loan.principal_paid is None or loan.interest_paid is None)
        ):

            # Get client info if available
            client = (
                db.query(Client).filter(Client.employee_id == loan.employee_id).first()
            )
            client_name = (
                f"{client.last_name} {client.other_names}" if client else "Unknown"
            )
...
def create_quality_issues_if_needed(db: Session, portfolio_id: int) -> Dict[str, int]:
        ...
        for group in duplicate_names:
            ...
            if existing_issue:
                ...
                existing_issue.description = (
                    f"Found {len(group)} clients with duplicate names"
                )
                db.flush()
            else:
                # Create new issue
    ...
    if duplicate_addresses:
        for group in duplicate_addresses:
            ...
            if existing_issue:
                ...
                existing_issue.affected_records = group
                existing_issue.description = (
                    f"Found {len(group)} clients with duplicate addresses"
                )
                db.flush()
            else:
                # Create new issue
    ...
    if missing_data:
        ...
        if missing_issue:
            ...
            missing_issue.description = (
                f"Found {len(missing_data)} loans with missing repayment data"
            )
            db.flush()
        else:
            # Create new issue
    ...
    existing_issues = (
        db.query(QualityIssue).filter(QualityIssue.portfolio_id == portfolio_id).all()
    )
