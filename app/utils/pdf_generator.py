...
def create_report_pdf(
    ...
    content.append(
        Paragraph(f"Generated on: {report_date.strftime('%B %d, %Y')}", subtitle_style)
    )
    ...
    if isinstance(report_data, dict):
        ...
        for key, value in report_data.items():
            if isinstance(value, dict):
                content.append(
                    Paragraph(f"{key.replace('_', ' ').title()}", subtitle_style)
                )

                # Create a table for the detailed section
                detail_data = []
                for sub_key, sub_value in value.items():
                    display_sub_key = " ".join(
    ...
    # If we have a list of items
        ...
        if report_data and isinstance(report_data[0], dict):
            ...
            display_headers = [
                " ".join(word.capitalize() for word in header.split("_"))
