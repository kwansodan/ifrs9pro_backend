...
async def process_loan_details(loan_details, portfolio_id, db):
    """Process loan details file using bulk operations."""
    try:
        ...
        # Get all existing loans for this portfolio in one query
        existing_loans = db.query(Loan).filter(Loan.portfolio_id == portfolio_id).all()
        existing_loans_dict = {loan.loan_no: loan for loan in existing_loans}
        ...
        for _, row in df.iterrows():
            try:
                ...
                if loan_no in existing_loans_dict:
                    # Update existing loan
                    existing_loan = existing_loans_dict[loan_no]
                    ...
                    loans_to_add.append(new_loan)
                
                rows_processed += 1
            except Exception as e:
                rows_skipped += 1
                # Consider using a proper logger instead of print
                print(f"Error processing loan row: {str(e)}")
...
async def process_loan_guarantees(loan_guarantee_data, portfolio_id, db):
    ...
    try:
        ...
        existing_guarantees = db.query(Guarantee).filter(
...
async def process_loan_collateral(loan_collateral_data, portfolio_id, db):
    ...
    try:
        ...
        existing_clients_dict = {client.employee_id: client for client in existing_clients}
