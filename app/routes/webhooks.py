...
async def paystack_webhook(
    ...
    if event_type == "charge.success":
    ...
    elif event_type in ("subscription.disable", "subscription.cancelled", "subscription.not_renew"):
        subscription_code = data.get("subscription_code")
        customer = data.get("customer", {}) or {}
        customer_email = customer.get("email")

        # Always resolve the admin user instead of the subscription's user
        admin_user = db.query(User).filter(User.role == "admin").first()
        if not admin_user:
            logger.error("No admin user found in DB to assign subscription changes")
        ...
        if subscription:
            ...
            # Link changes to admin user
            admin_user.subscription_status = "cancelled"
            admin_user.current_subscription_id = None
            admin_user.paystack_customer_code = customer.get("customer_code") or admin_user.paystack_customer_code
            db.add(admin_user)

            db.commit()
            logger.info(f"Subscription {subscription_code} cancelled for admin {admin_user.email}")
